...
bool c_render::init()
{
	...
	constexpr ImWchar icon_ranges[] = { 0xE000, 0xF8FF, 0 };
	{
		...
		fonts.pixel.init(io, smallest_pixel_7, sizeof(smallest_pixel_7), 10.f, &cfg, io.Fonts->GetGlyphRangesCyrillic());
                ...
                cfg.MergeMode = true;
                cfg.DstFont = fonts.eventlog.get();
                fonts.eventlog.init(io, "C:\\Windows\\Fonts\\msyh.ttc", 10.f, &cfg, io.Fonts->GetGlyphRangesChineseSimplifiedCommon());
                cfg.MergeMode = false;
                cfg.DstFont = nullptr;
	}
	...
	//	cfg.RasterizerFlags = ImGuiFreeType::RasterizerFlags::ForceAutoHint;
        ...
        cfg.MergeMode = true;
        cfg.DstFont = fonts.main.get();
        fonts.main.init(io, "C:\\Windows\\Fonts\\msyh.ttc", 14.f, &cfg, io.Fonts->GetGlyphRangesChineseSimplifiedCommon());
        cfg.MergeMode = false;
        cfg.DstFont = nullptr;
        fonts.large.init(io, SFUIDisplay_SemiBold, sizeof(SFUIDisplay_SemiBold), 30.f, &cfg, io.Fonts->GetGlyphRangesCyrillic());
        cfg.MergeMode = true;
        cfg.DstFont = fonts.large.get();
        fonts.large.init(io, "C:\\Windows\\Fonts\\msyh.ttc", 30.f, &cfg, io.Fonts->GetGlyphRangesChineseSimplifiedCommon());
        cfg.MergeMode = false;
        cfg.DstFont = nullptr;
        fonts.esp.init(io, CXOR("C:\\Windows\\Fonts\\verdana.ttf"), 12.f, &cfg, io.Fonts->GetGlyphRangesCyrillic());
        cfg.MergeMode = true;
        cfg.DstFont = fonts.esp.get();
        fonts.esp.init(io, "C:\\Windows\\Fonts\\msyh.ttc", 12.f, &cfg, io.Fonts->GetGlyphRangesChineseSimplifiedCommon());
        cfg.MergeMode = false;
        cfg.DstFont = nullptr;
        fonts.misc.init(io, SFUIDisplay_SemiBold, sizeof(SFUIDisplay_SemiBold), 14.f, &cfg, io.Fonts->GetGlyphRangesCyrillic());
        cfg.MergeMode = true;
        cfg.DstFont = fonts.misc.get();
        fonts.misc.init(io, "C:\\Windows\\Fonts\\msyh.ttc", 14.f, &cfg, io.Fonts->GetGlyphRangesChineseSimplifiedCommon());
        cfg.MergeMode = false;
        cfg.DstFont = nullptr;
        fonts.bold.init(io, SFUIDisplay_Bold, sizeof(SFUIDisplay_Bold), 10.f, &cfg, io.Fonts->GetGlyphRangesCyrillic());
        cfg.MergeMode = true;
        cfg.DstFont = fonts.bold.get();
        fonts.bold.init(io, "C:\\Windows\\Fonts\\msyh.ttc", 10.f, &cfg, io.Fonts->GetGlyphRangesChineseSimplifiedCommon());
        cfg.MergeMode = false;
        cfg.DstFont = nullptr;
        fonts.bold2.init(io, SFUIDisplay_Bold, sizeof(SFUIDisplay_Bold), 14.f, &cfg, io.Fonts->GetGlyphRangesCyrillic());
        cfg.MergeMode = true;
        cfg.DstFont = fonts.bold2.get();
        fonts.bold2.init(io, "C:\\Windows\\Fonts\\msyh.ttc", 14.f, &cfg, io.Fonts->GetGlyphRangesChineseSimplifiedCommon());
        cfg.MergeMode = false;
        cfg.DstFont = nullptr;
        fonts.bold_large.init(io, SFUIDisplay_Bold, sizeof(SFUIDisplay_Bold), 26.f, &cfg, io.Fonts->GetGlyphRangesCyrillic());
        cfg.MergeMode = true;
        cfg.DstFont = fonts.bold_large.get();
        fonts.bold_large.init(io, "C:\\Windows\\Fonts\\msyh.ttc", 26.f, &cfg, io.Fonts->GetGlyphRangesChineseSimplifiedCommon());
        cfg.MergeMode = false;
        cfg.DstFont = nullptr;
        fonts.dmg.init(io, SFUIDisplay_SemiBold, sizeof(SFUIDisplay_SemiBold), 18.f, &cfg, io.Fonts->GetGlyphRangesCyrillic());
        cfg.MergeMode = true;
        cfg.DstFont = fonts.dmg.get();
        fonts.dmg.init(io, "C:\\Windows\\Fonts\\msyh.ttc", 18.f, &cfg, io.Fonts->GetGlyphRangesChineseSimplifiedCommon());
        cfg.MergeMode = false;
        cfg.DstFont = nullptr;
